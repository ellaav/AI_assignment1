        ...
        if self.Terminate == False:
            if self.Turns_To_Wait == 0:
                ...
                if path[0]==0:
                    ...
                    self.State = path[0]
                    #self.State, weightpath = Go_In_Path(graph2 , path ,state)
                    #self.Turns_To_Wait = weightpath 
                    self.People_To_Save = number_of_people_in_the_graph(self.Graph)
...
def gready(graph,node):
    #number of people in the graph is a fuction to know when we finish the task
    ...
    while people_in_the_graph > 0:
        ...
        path = Get_Path_Nearest_Goal(graph2)
        #Go_In_Path is a function that do the walk in the path and return the final state and the cost of all the steps
...
def Get_Path_Nearest_Goal(graph):
    ...
    for i in vertex:
        ...
        if (count <= minlength):
            minlength = count
            the_chosen_vertex = i
            # print("the chosen vertex:" , the_chosen_vertex)
            # print ("the path long: " , count)
